...
URL 参数（path/line）、seek 机制与档位切换规则、`window.__storyRec` 画布抓帧器、
字体代理、已知限制、依赖的引擎钩子（`setDecisionPolicy` /
`ConditionStore.satisfyingAssignment` / `getPlayer`）与坑位清单——调试页的全部
文档直接写在 `debug/StoryPlayer.html` 文件顶部的注释里，改动调试相关行为时同步
更新那份注释。
