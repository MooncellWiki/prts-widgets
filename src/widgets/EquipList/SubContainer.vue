...
<template>
  <NCard class="w-full lg:w-[calc(50%-0.5rem)] m-1" header-style="text-align:center;" size="small" hoverable>
    <template #header>
      <div class="display-flex flex-items-center flex-justify-center">
        <span class="inline-flex w-[40px] h-[40px] vertical-text-bottom overflow-hidden justify-center flex-items-center bg-black">
          <img :src="src" style="position:absolute;transform:scale(0.5);">
        </span>
        <span class="text-center inline-block" style="width:6em">{{ title }}</span>
      </div>
    </template>
